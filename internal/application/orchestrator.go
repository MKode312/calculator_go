...
type Task struct {
	...
	Node          *calc.ASTNode `json:"-"`
}

func checkExpressionErrors(node *calc.ASTNode) error {
	if node == nil {
		return calc.ErrEmptyExpression
	}

	// Проверка серией рекурсивных вызовов
...
func validateNode(node *calc.ASTNode) error {
	if node.IsLeaf {
		return nil // Листовой узел, ошибок нет
	}

	// Проверка операторов
	if node.Operator != "+" && node.Operator != "-" && node.Operator != "*" && node.Operator != "/" {
		return calc.ErrInvalidOp
	}

	// Проверка на наличие поддеревьев
	...
	if node.Left != nil && !node.Left.IsLeaf {
		if node.Left.Operator != "" {
			return calc.ErrTwoConsecutiveOps
		}
	}
	...
	// Рекурсивная проверка левой и правой стороны
...
func (o *Orchestrator) calculateHandler(w http.ResponseWriter, r *http.Request) {
	...
	ast, err := calc.ParseAST(req.Expression)
	if err != nil {
...
func (o *Orchestrator) getTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	if expr, exists := o.exprStore[task.ExprID]; exists {
		// Добавляем проверку ошибок перед изменением статуса
...
func (o *Orchestrator) scheduleTasks(expr *Expression) {
	var traverse func(node *calc.ASTNode)
	traverse = func(node *calc.ASTNode) {
